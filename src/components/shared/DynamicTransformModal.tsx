...
export function DynamicTransformModal({
  ...
  const isFieldKey = (fieldName: string) => {
    if (!fieldName) return false;
    const clean = fieldName.trim().toLowerCase();
    return keyColumns.some(k => k.trim().toLowerCase() === clean || clean.includes(k.trim().toLowerCase()));
  };
  ...
  const handleRun = async () => {
      ...
      try {
        if (projectId) {
          ...
          if (res.ok) {
            ...
            realPythonScript = data.python_code || data.ai_rules?.[0]?.Target_Data || data.summary?.ai_rules?.[0]?.Target_Data || '';
          }
        }
      } catch (err) {
    ...
    import re
    # Transform field ${selectedField}
    cols = [c for c in df.columns if str(c).strip().lower() == '${selectedField.toLowerCase()}']
    if not cols:
        cols = ['${selectedField}']
    for col in cols:
    ...
    return df`;
      ...
      newRule = {
        ...
        prompt: aiPrompt.trim() || `Transform ${selectedField}`,
        enabled: true,
        active: true,
        pythonCode: realPythonScript,
        python_code: realPythonScript,
      };
    }
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
        ...
        {/* Modal Window — Solid Background */}
        <motion.div
          initial={{ opacity: 0, scale: 0.96, y: 10 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.96, y: 10 }}
          transition={{ duration: 0.2, ease: [0.16, 1, 0.3, 1] }}
          className="relative w-full max-w-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-2xl overflow-hidden z-10 flex flex-col max-h-[90vh]"
        >
          {/* Header */}
          <div className="p-4 sm:p-5 border-b border-gray-100 dark:border-gray-800 bg-gradient-to-r from-violet-50/70 via-white to-white dark:from-violet-950/20 dark:via-gray-900 dark:to-gray-900 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-3">
              <div className="p-2.5 rounded-xl bg-violet-100 dark:bg-violet-900/40 text-violet-600 dark:text-violet-400 border border-violet-200 dark:border-violet-800">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {tableName && (
                    <span className="px-2 py-0.5 rounded-md text-[10px] font-mono font-bold bg-violet-100 dark:bg-violet-900/50 text-violet-700 dark:text-violet-300 border border-violet-200 dark:border-violet-800">
                      {tableName}
                    </span>
                  )}
                  {rowNumber !== undefined && (
                    <span className="px-2 py-0.5 rounded-md text-[10px] font-mono font-bold bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
                      Row #{rowNumber}
                    </span>
                  )}
                </div>
          ...
          {/* Body Content */}
          <div className="p-5 overflow-y-auto space-y-5 flex-1 scrollbar-thin bg-white dark:bg-gray-900">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between text-xs font-bold text-gray-700 dark:text-gray-300">
                ...
                {selectedField && isFieldKey(selectedField) && (
                  <span className="inline-flex items-center gap-1 text-[11px] font-bold text-amber-600 dark:text-amber-400">
                    <Key className="w-3 h-3" /> Key Field
                  </span>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-2">
                ...
                <div className="sm:col-span-5 relative">
                  ...
                  <input
                    ...
                    className="w-full text-xs pl-8 pr-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:bg-white dark:focus:bg-gray-800"
                  />
                </div>
                ...
                <div className="sm:col-span-7">
                  <select
                    ...
                    className="w-full text-xs font-mono font-bold px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-violet-500"
                  >
                    {filteredFields.map((f) => (
                      <option key={f} value={f}>
                        {f} {isFieldKey(f) ? '🔑 (Key Field)' : ''}
                      </option>
                    ))}
                  </select>
                </div>
            ...
            <div className="flex rounded-xl bg-gray-100 dark:bg-gray-800 p-1 border border-gray-200 dark:border-gray-700">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-1.5 px-3 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  mode === 'presets'
                    ? 'bg-white dark:bg-gray-900 text-violet-600 dark:text-violet-400 shadow-xs'
            ...
            {mode === 'presets' && (
              <div className="space-y-4 p-4 rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/40">
                <span className="text-[11px] font-bold text-gray-700 dark:text-gray-300">Choose Quick Operation</span>
                    ...
                    <button
                      ...
                      className={`p-2.5 rounded-xl border text-left transition-all cursor-pointer ${
                        selectedPreset === preset.id
                          ? 'border-violet-500 bg-violet-50 dark:bg-violet-950/50 text-violet-700 dark:text-violet-300 font-bold shadow-xs'
                ...
                {['prefix', 'suffix', 'default_if_empty'].includes(selectedPreset) && (
                  <div className="space-y-1 pt-1">
                    ...
                    <input
                      ...
                      className="w-full text-xs font-mono px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-violet-500"
                    />
                  </div>
                )}
              </div>
            ...
            {mode === 'ai' && (
              <div className="space-y-3 p-4 rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/40">
                <div className="space-y-1">
                  ...
                  <textarea
                    ...
                    placeholder={`Describe transformation for ${selectedField}...\n\nExamples:\n• "If ${selectedField} starts with 'US', replace with 'USA'"\n• "Remove hyphens and special characters from ${selectedField}"\n• "Pad with leading zeros to make required length"`}
                    className="w-full h-24 p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-xs text-gray-900 dark:text-gray-100 resize-none focus:outline-none focus:ring-2 focus:ring-cyan-500 placeholder:text-gray-400 dark:placeholder:text-gray-500"
                  />
                </div>
              </div>
            )}
            ...
            <div className="p-3.5 rounded-xl border border-violet-200 dark:border-violet-900 bg-violet-50/60 dark:bg-violet-950/30 space-y-1">
              <div className="flex items-center gap-1.5 text-[10.5px] font-bold uppercase tracking-wider text-violet-600 dark:text-violet-400">
                <Sparkles className="w-3.5 h-3.5" />
                <span>Live Rule Preview</span>
              </div>
              <p className="text-xs font-mono font-bold text-gray-900 dark:text-gray-100 leading-relaxed">
                {ruleSummary}
              </p>
            </div>
          </div>
          ...
          {/* Footer Actions */}
          <div className="p-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50/80 dark:bg-gray-800/60 flex items-center justify-between shrink-0">
            <Button variant="secondary" onClick={onClose} disabled={isApplying || isGeneratingAI}>
              Cancel
            </Button>
            ...
            <Button
              ...
              onClick={handleRun}
              disabled={isApplying || isGeneratingAI || !selectedField}
              className="px-5 font-bold"
            >
              {isGeneratingAI ? 'Generating Script...' : isApplying ? 'Running...' : 'Run Transform on Spot'}
            </Button>
          </div>
        </motion.div>
      </div>
