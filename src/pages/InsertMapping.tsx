...
export function InsertMapping() {
  ...
  return (
    <PageLayout>
      <PageGrid>
        <GridCol span={12}>
          <PageHeader title="Source Field Data Dictionary Entry" subtitle="Directly insert custom Oracle EBS to SAP mappings into the database">
          ...
          <Card className="mb-6 overflow-visible z-20">
            <CardHeader title="1. Select Context" subtitle="Choose the source system and target SAP object" />
            <CardBody className="p-4 grid grid-cols-2 gap-6 overflow-visible">
              ...
              <div>
                <label className="block text-[11px] font-bold text-[var(--text-secondary)] mb-1.5 uppercase tracking-wider">Target SAP Object</label>
                <Select
                ...
                {rows.map((row, i) => (
                  <div key={row.id} className="grid grid-cols-[1fr_1fr_1fr_40px] gap-4 items-center bg-[var(--bg-tertiary)] p-2 rounded-xl border border-[var(--border-light)]">
                    <Select
                      ...
                      options={[{ value: '', label: 'Select SAP Field...' }, ...sapFields.map(f => ({ value: f.id, label: `${f.sap_structure}.${f.field_name} - ${f.field_description}` }))]}
