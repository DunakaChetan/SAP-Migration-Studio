...
export function Step9TechDocs() {
  ...
  const targetObject = state.obj || persistedReport?.target_object || 'CUSTOMER';
  const sourceSystem = state.src || persistedReport?.source || 'ORACLE_EBS';

  const projectName =
    state.projectName ||
    persistedReport?.project_name ||
    (state.projectId ? 'SAP S/4HANA Migration Project' : 'Global SAP Migration');
  ...
  useEffect(() => {
    setEmailSubject(`SAP S/4HANA Migration Audit Report — ${objectDisplayName} (${sourceDisplayName})`);
