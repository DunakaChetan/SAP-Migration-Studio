...
class SaveTechDocRequest(BaseModel):
    project_id: str
    target_object: str
    source: Optional[str] = "ORACLE_EBS"
...
def resolve_source_system(client, source_key: str) -> tuple[Optional[str], str]:
    ...
    try:
        ...
        # Check by partial match (e.g. ORACLE, SAP)
        part = "ORACLE" if "ORACLE" in clean_key else ("SAP" if "SAP" in clean_key else clean_key)
...
def save_tech_doc(req: SaveTechDocRequest):
    ...
    # 4. Reuse existing ID if already available so only ONE record exists!
    if existing_doc:
    ...
    record_payload = {
        ...
        "object_id": obj_id,
        "source_id": source_id,
        "source": req.source or "ORACLE_EBS",
    ...
    try:
        ...
        if "source_id" in err_str or "column" in err_str:
            # Fallback for table if user has not yet recreated schema with source_id column
...
def send_tech_doc_email(req: SendEmailRequest):
    ...
    client = supabase_service.get_client()
    proj_display = req.project_name or resolve_project_name(client, req.project_id) or "SAP S/4HANA Migration"
    obj_display = req.object_name or OBJECT_DISPLAY_NAMES.get((req.target_object or "CUSTOMER").upper(), req.target_object or "Customer Master")
    src_display = req.source_name or SOURCE_DISPLAY_NAMES.get((req.source or "ORACLE_EBS").upper(), req.source or "Oracle EBS R12")

    subject = req.subject or f"SAP Migration Studio — Consolidated Audit Report [{obj_display}] — {proj_display}"
    notes = req.notes or "Please find attached the official SAP Migration Studio Consolidated Master & Post-Load Audit Report."
    ...
    sender_email = os.environ.get("SMTP_FROM", smtp_user or "reports@sap-migration-studio.com")
...
==================================================
MIGRATION CONSOLIDATED EXECUTIVE SUMMARY
==================================================
• Project: {proj_display}
• Target Object: {obj_display}
• Source System: {src_display}
• Audit Status: Verified 100% S/4HANA Ready (0 Blocking Errors)
• Generated At: {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')}
• View Live Report: {req.report_url or 'Open in SAP Migration Studio'}
==================================================

Attached Files:
1. Consolidated_Master_Report.pdf (Vector Printable Audit Dossier)
2. Consolidated_Master_Metrics.csv (Machine-Readable Pipeline Data)

Generated securely by SAP Migration Studio (S/4HANA Edition).
"""
