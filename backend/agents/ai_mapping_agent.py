...
class AIMappingAgent:
    def map_source_to_target(self, source_system: str, target_object: str, known_source_fields: list, target_fields: list):
        if known_source_fields:
            # Source Fields provided (either SAP ECC or File Upload) -> Map to Target Dictionary
            system_prompt = f"""You are an expert ERP Data Migration Architect with deep knowledge of {source_system} and SAP S/4HANA.
You need to accurately map a specific list of Source Fields from {source_system} to a provided dictionary of SAP S/4HANA target fields for the object {target_object}.
...
You MUST heavily rely on the 'description' to deeply understand the true semantic meaning of the SAP field before matching it to a Source Field.
If a source field has NO logical matching SAP target field based on the descriptions, DO NOT map it. Do NOT hallucinate target fields like "None", "N/A", or "null". Simply omit that source field from the output array entirely.
...
3. COMMON SEMANTIC MATCHES: 
   - Phone / Mobile -> `S_ADDRESS.TELNR_LONG`, `S_ADDRESS.TELNR_LONG_2`, etc.
   - Email -> `S_ADDRESS.SMTP_ADDR`
   - Street / Address -> `S_ADDRESS.STREET`
   - Postal Code / ZIP -> `S_ADDRESS.POST_CODE1`
   - City -> `S_ADDRESS.CITY`
   - Tax ID / VAT -> `S_CUST_TAXNUMBERS.TAXNUM` or `STCD1`
   - Name -> `S_CUST_GEN.NAME_FIRST`, `S_CUST_GEN.NAME_LAST`
...
"""
            ...
            # Oracle EBS / Other Behavior: Loop over Target Fields and hallucinate source fields
            system_prompt = f"""You are an expert ERP Data Migration Architect with deep knowledge of {source_system} and SAP S/4HANA.
You need to accurately map fields from {source_system} to SAP S/4HANA target object {target_object}.
You are given a STRICT list of MANDATORY target fields in SAP formatted as {{"sap_field": "STRUCTURE.FIELD_NAME", "description": "Human readable description"}}. 
CRITICAL REQUIREMENT: You MUST provide EXACTLY ONE mapping for EVERY SINGLE field in the target list. Do NOT omit any fields. If there are duplicate field names (e.g. KUNNR), you must map each one individually because they belong to different SAP structures. Do NOT summarize or truncate.
Because the known source fields list is empty, you MUST act as a Senior Architect and perform a deep internal knowledge search to find the EXACT standard database table and column names in {source_system} (e.g., for Oracle EBS, use standard tables like HZ_PARTIES, AP_SUPPLIERS) that correspond to each SAP target field.
